...
# 回顾
做这个是因为目前的效果很不好，我想回顾我做了什么，然后看看有什么与原本不相符，从而修改。

首先是网络架构，backbone由于我自己训练资源不够，所以我获取的是网上的资源，直接加载权重，分类头是我的，我修改backbone后接上了。我认为这个架构应该是没有什么问题的。然后就是训练问题。
训练时对于每张图片我会缩放裁剪成320*320，同时同步更新标注，将图片输入网络后首先对每个层次的输出分别处理，然后对一个Batchsize里的每个单独处理，每次处理我会先计算标注和输出框的IoU，然后获取每个标注框最大的IoU输出框，将其设定为正样本，同时将所有IoU低于阈值的（此值我开始设为0.7，现根据论文改成0.5）设为负样本，然后计算正样本的和标注框四个值的MSE，正负样本置信度分别和全一，全零的BCE，正样本类别和独热的BCE，对于他们的权重分别给了p_box=1.0, p_obj=1.0, p_noobj=0.5, p_class=1.0然后反向传播，每个batch一次。优化器我使用了Adam，lr定为1e-3，我目前只在乎明显的效果，日后可以换成SGD。训练中我冻结了backbone的权重。
可视化的结果很差，可视化我是将输出得到框，去掉置信度小于0.5的，然后做nms处理得到剩余的框，然后在图片上标注，保存。
我在这两天首先在阈值设置为0.7以下为负样本，冻结了backbone的权重，训练了一百四十四个epoch，效果很糟糕，具体而言是可视化出的结果有密密麻麻的框，各个层次都是，小grid cell的层更是如此，同时很少看到和标注一样的正确的框同时是正确的类别。我准备修改阈值到0.5，不再冻结权重，在原本的基础上继续训练。或许目前还存在什么明显的错误，比如我曾看到一种损失函数的设计在正样本框的MSE乘（2-w*h）作为惩罚小框，或许权重错了。
现在我已经改了负样本阈值到0.5，不再冻结backbone权重，继续训练，启用了nms，然后在小数据集上训练，在val数据集，一张图片，20张图片，一张图片能够降低损失到0但仍有很多框，密密麻麻，其他的不仅有很多框损失在300个epoch也没有收敛到一个较低的值。我尝试增加noobj的损失系数到5，一张图片的框少了，但是结果仍然不完全拟合标注。在20张图则完全看不到预测框了，损失的表现也不好。